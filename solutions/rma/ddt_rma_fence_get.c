...
int main(int argc, char **argv)
{
    ...
    for (iter = 0; iter < niters; ++iter) {
        ...
        MPI_Get(&aold[ind(1, 0)], 1, north_south_type, north,
                ind(1, by) + offset, 1, north_south_type, win);

        MPI_Get(&aold[ind(1, by + 1)], 1, north_south_type, south,
                ind(1, 1) + offset, 1, north_south_type, win);
