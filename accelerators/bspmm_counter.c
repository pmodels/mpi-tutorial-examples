 ...
 * the submatrix of C that it computes. The dgemm multiplication is accelerated using
 * OpenAcc.
 */
...
#pragma acc data create (local_a[0:BLK_DIM*BLK_DIM],local_b[0:BLK_DIM*BLK_DIM],\
                         local_c[0:BLK_DIM*BLK_DIM])
    do {
...
#pragma acc data update device(local_a[0:BLK_DIM*BLK_DIM],local_b[0:BLK_DIM*BLK_DIM])

            /* offload computation to the device in dgemm_acc() */
            /* compute Cij += Aik * Bkj only if both local_a and local_b are nonzero */
            dgemm_acc(local_a, local_b, local_c);

#pragma acc data update host(local_c[0:BLK_DIM*BLK_DIM])
...
#pragma acc parallel loop present(local_a, local_b, local_c)
    for (j = 0; j < BLK_DIM; j++) {
        for (i = 0; i < BLK_DIM; i++) {
            local_c[j + i * BLK_DIM] = 0.0;
            for (k = 0; k < BLK_DIM; k++)
